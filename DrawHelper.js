...
var DrawHelper = (function() {
    ...
    function _(cesiumWidget) {
		...
		this._tooltipLav = createTooltipLav(cesiumWidget.container);
        this._surfaces = [];
    ...
    _.prototype.registerEditableShape = function(surface) {
        ...
        setListener(surface, 'mouseMove', function(position) {
            surface.setHighlighted(true);_self._tooltip.setVisible(false);
...
//
        ...
        if(callbacks) {
            ...
            if(callbacks.tooltip) {
                setListener(billboard, 'mouseMove', function(position) {
					_self._drawHelper._tooltip.setVisible(true);
	...
	function getExtentCornersWhileDrawing(value) {
		...
		Cesium.Rectangle.southeast(value,southeast);
		Cesium.Rectangle.southwest(value,southwest);
		Cesium.Rectangle.northeast(value,northeast);
		Cesium.Rectangle.northwest(value,northwest);
		...
		var obj = {
			southeastlonDegrees:  southeastlonDegrees,
			southeastlatDegrees: southeastlatDegrees,
			southwestlonDegrees:  southwestlonDegrees,
			southwestlatDegrees: southwestlatDegrees,
			northeastlonDegrees:  northeastlonDegrees,
			northeastlatDegrees: northeastlatDegrees,
			northwestlonDegrees:  northwestlonDegrees,
			northwestlatDegrees: northwestlatDegrees,
			ABDIST: totalDistanceInKmAB,
			ACDIST: totalDistanceInKmAC
		};
        return obj;//[southeastlonDegrees, southeastlatDegrees,southwestlonDegrees,southwestlatDegrees,northeastlonDegrees,northeastlatDegrees,northwestlonDegrees,northwestlatDegrees,totalDistanceInKmAB,totalDistanceInKmAC];
    }
    ...
    _.prototype.startDrawingExtent = function(options) {
        ...
        var tooltip = this._tooltip;
		var tooltipLav = this._tooltipLav;
        ...
        function updateExtent(value) {
			...
			if(extent == null) { extent = new DrawHelper.ExtentPrimitive({ extent : value, material : options.material, }); extent.asynchronous = false; primitives.add(extent); } 
        ...
        mouseHandler.setInputAction(function(movement) {
            if(movement.position != null) {
                var cartesian = scene.camera.pickEllipsoid(movement.position, ellipsoid);
				console.log("eimai i start extent kai to movement position einai: "+movement.position);
                if (cartesian) {
                    if(extent == null) {
                        // create the rectangle
						console.log("create the rectangle");
        ...
        mouseHandler.setInputAction(function(movement) {
            ...
            if(position != null) {
                if(extent == null) {
					...
					tooltipLav.showAt(position, "<p>Click to start drawing rectangle</p>");
                } else {
                    var cartesian = scene.camera.pickEllipsoid(position, ellipsoid);
					console.log("eimai i telos extent kai to firstPoint einai: "+firstPoint+" kai to ellipsoid.cartesianToCartographic(cartesian)"+ellipsoid.cartesianToCartographic(cartesian));
                    if (cartesian) {
                        var value = getExtent(firstPoint, ellipsoid.cartesianToCartographic(cartesian));
						var obj=getExtentCornersWhileDrawing(value);
						var celldiv=document.getElementById("cellsize1new");//cellsize1new
						var cellSize=celldiv.value;
						var rows=Math.round((obj.ACDIST*1000)/cellSize);
						var cols=Math.round((obj.ABDIST*1000)/cellSize);
                        updateExtent(value);
                        tooltip.showAt(position, "<p>Drag to change rectangle extent</p><p>Click again to finish drawing: </p>"+ obj.ABDIST.toFixed(2));
						//tooltipLav.showAt(position, "Distnace AB: "+ obj.ABDIST.toFixed(2)+"km and Distance AC: "+obj.ACDIST.toFixed(2)+"km and cellsize: "+cellSize+". So, "+rows+" rows and "+cols+" cols!");
                        tooltipLav.showAt(position, rows+"rows x "+cols+"cols -"+obj.ABDIST.toFixed(2)+"km x "+obj.ACDIST.toFixed(2)+"km ,cellsize: "+cellSize+" \n :::southeastlonDegrees:"+southeastlonDegrees+" southeastlatDegrees:"+southeastlatDegrees+" \n southwestlonDegrees"+southwestlonDegrees+" southwestlatDegrees:"+southwestlatDegrees+" \n northeastlonDegrees:"+northeastlonDegrees+" northeastlatDegrees"+northeastlatDegrees+" \nnorthwestlonDegrees:"+northwestlonDegrees+" northwestlatDegrees:"+northwestlatDegrees);
        ...
        function setEditMode(editMode) {
                ...
                if(editMode) {
                    ...
                    if(this._markers == null) {
                         markers = new _.BillboardGroup(drawHelper, dragBillboard);
                         editMarkers = new _.BillboardGroup(drawHelper, dragHalfBillboard);
                        ...
                        var handleMarkerChanges = {
                            dragHandlers: {
                                onDrag: function(index, position) {
									...
									//tooltipLav.showAt(position, rows+"rows x "+cols+"cols -"+obj.ABDIST.toFixed(2)+"km x "+obj.ACDIST.toFixed(2)+"km ,cellsize: "+cellSize+" \n :::southeastlonDegrees:"+southeastlonDegrees+" southeastlatDegrees:"+southeastlatDegrees+" \n southwestlonDegrees"+southwestlonDegrees+" southwestlatDegrees:"+southwestlatDegrees+" \n northeastlonDegrees:"+northeastlonDegrees+" northeastlatDegrees"+northeastlatDegrees+" \nnorthwestlonDegrees:"+northwestlonDegrees+" northwestlatDegrees:"+northwestlatDegrees);
                                    tooltipLav.showAt(position, "<p>Click to start editing rectangle</p>");
...
//var markers;
        DrawHelper.ExtentPrimitive.prototype.setEditable = function(e) {
            ...
            var extent = this;
            var scene = drawHelper._scene;
			//var tooltipLav1 = createTooltipLav(cesiumWidget.container);
            drawHelper.registerEditableShape(extent);
            extent.asynchronous = false;
			console.log("mporo na to doulepso edo?");
			//tooltipLav.showAt(position, rows+"rows x "+cols+"cols -"+obj.ABDIST.toFixed(2)+"km x "+obj.ACDIST.toFixed(2)+"km ,cellsize: "+cellSize+" \n :::southeastlonDegrees:"+southeastlonDegrees+" southeastlatDegrees:"+southeastlatDegrees+" \n southwestlonDegrees"+southwestlonDegrees+" southwestlatDegrees:"+southwestlatDegrees+" \n northeastlonDegrees:"+northeastlonDegrees+" northeastlatDegrees"+northeastlatDegrees+" \nnorthwestlonDegrees:"+northwestlonDegrees+" northwestlatDegrees:"+northwestlatDegrees);

										var value = e;//getExtent(ellipsoid.cartesianToCartographic(corner), ellipsoid.cartesianToCartographic(position));//getExtent(firstPoint, ellipsoid.cartesianToCartographic(cartesian));
										var obj=getExtentCornersWhileDrawing(extentfordraw);
										var celldiv=document.getElementById("cellsize1new");//cellsize1new
										var cellSize=celldiv.value;
										//var rows=Math.round((obj.ACDIST*1000)/cellSize);
										//var cols=Math.round((obj.ABDIST*1000)/cellSize);
										var victorAB=distance(obj.southeastlatDegrees, obj.southeastlonDegrees, obj.southwestlatDegrees, obj.southwestlonDegrees, 'K');
										var victorAC=distance(obj.southeastlatDegrees, obj.southeastlonDegrees, obj.northeastlatDegrees, obj.northeastlonDegrees, 'K');
										var rows=Math.round((victorAC*1000)/cellSize);
										var cols=Math.round((victorAB*1000)/cellSize);
										//updateExtent(value);
										//tooltipLav.showAt(position, "Distnace AB: "+ obj.ABDIST.toFixed(2)+"km and Distance AC: "+obj.ACDIST.toFixed(2)+"km and cellsize: "+cellSize+". So, "+rows+" rows and "+cols+" cols!");
										//tooltipLav.showAt('koukou', rows+"rows x "+cols+"cols -"+victorAB.toFixed(2)+"km x "+victorAC.toFixed(2)+"km ,cellsize: "+cellSize);
										totalDistanceInKmAB=victorAB;
										totalDistanceInKmAC=victorAC;
										tooltipLav.showAt('koukou',"cell: "+cellSize+"m. - grid: "+rows+" x "+cols+" - size: "+victorAB.toFixed(2)+" x "+victorAC.toFixed(2)+"km");
										//tooltipLav.showAt('koukou', rows+"rows x "+cols+"cols -"+obj.ABDIST.toFixed(2)+"km x "+obj.ACDIST.toFixed(2)+"km ,cellsize: "+cellSize+" \n :::southeastlonDegrees:"+southeastlonDegrees+" southeastlatDegrees:"+southeastlatDegrees+" \n southwestlonDegrees"+southwestlonDegrees+" southwestlatDegrees:"+southwestlatDegrees+" \n northeastlonDegrees:"+northeastlonDegrees+" northeastlatDegrees"+northeastlatDegrees+" \nnorthwestlonDegrees:"+northwestlonDegrees+" northwestlatDegrees:"+northwestlatDegrees);
                        ...
                        var handleMarkerChanges = {
                            dragHandlers: {
                                onDrag: function(index, position) {
									...
									//tooltipLav.showAt(position, "<p>Click to start editing rectangle</p>");
									//var koukoukou=document.getElementsByClassName("loggingpuretest");
									//koukoukou.innerHTML="KOUKOUKOU";
									//console.log("<p>Click1 to start editing rectangle</p> pos"+position);
									...
									//if (cartesian) {
										console.log("mpika");
										var value = getExtent(ellipsoid.cartesianToCartographic(corner), ellipsoid.cartesianToCartographic(position));//getExtent(firstPoint, ellipsoid.cartesianToCartographic(cartesian));
										var obj=getExtentCornersWhileDrawing(value);
										var celldiv=document.getElementById("cellsize1new");//cellsize1new
										var cellSize=celldiv.value;
										var rows=Math.round((obj.ACDIST*1000)/cellSize);
										...
										var victorAB=distance(obj.southeastlatDegrees, obj.southeastlonDegrees, obj.southwestlatDegrees, obj.southwestlonDegrees, 'K');
										var victorAC=distance(obj.southeastlatDegrees, obj.southeastlonDegrees, obj.northeastlatDegrees, obj.northeastlonDegrees, 'K');
										var rowsVIC=Math.round((victorAC*1000)/cellSize);
										var colsVIC=Math.round((victorAB*1000)/cellSize);
										totalDistanceInKmAB=victorAB;
										totalDistanceInKmAC=victorAC;
										//updateExtent(value);
										//tooltipLav.showAt(position, "Distnace AB: "+ obj.ABDIST.toFixed(2)+"km and Distance AC: "+obj.ACDIST.toFixed(2)+"km and cellsize: "+cellSize+". So, "+rows+" rows and "+cols+" cols!");
										tooltipLav.showAt(position, "cell: "+cellSize+"m. - grid: "+rowsVIC+" x "+colsVIC+" - size: "+victorAB.toFixed(2)+" x "+victorAC.toFixed(2)+"km");//+" and AB victor distance(km) is :"+victorAB.toFixed(2)+" and AC victor distance(km) is :"+victorAC.toFixed(2)+"  rowsVIC:"+rowsVIC+" colsVIC"+colsVIC);
									//}
                                },
                                onDragEnd: function(index, position) {
                                    onEdited();
	...
	//DrawHelperWidgetBTNperimeter
	 _.DrawHelperWidgetBTNperimeter = (function() {
        ...
        function _(drawHelper, options) {
            ...
            var add_perimeter = document.createElement('DIV');
            add_perimeter.className = "add_perimeter";
            options.container.appendChild(add_perimeter);

            function addIcon(id, url, title, callback) {
			var thakoimitholes=document.getElementById("add_perimeter");
				//alert("de paizei na koimitho: "+thakoimitholes.name);
				thakoimitholes.onclick = callback;
                //var div = document.createElement('submit');
                //--div.className = 'flamebutton';
				//div.style='float:left; margin-right:10px;'
                //--div.title = title;
				//div.value='Add ignition point';
                //toolbar11.appendChild(div);
 ...
 _.DrawHelperWidgetBTN = (function() {
        ...
        function _(drawHelper, options) {
            ...
            var toolbar11 = document.createElement('DIV');
            toolbar11.className = "toolbar11";
            options.container.appendChild(toolbar11);

            function addIcon(id, url, title, callback) {
			var thakoimitholes=document.getElementById("toolbar11");
				//alert("de paizei na koimitho: "+thakoimitholes.name);
				thakoimitholes.onclick = callback;
                //var div = document.createElement('submit');
                //--div.className = 'flamebutton';
            ...
            addIcon('marker',options.markerIcon, 'Click to start drawing a 2D marker', function() {
                drawHelper.startDrawingMarker({
                    callback: function(position) {
								//alert("Add ignition point");
								var igntxtbtn=document.getElementById("toolbar11");
								igntxtbtn.value="Add ignition point";
                        _self.executeListeners({name: 'markerCreated', position: position});
                    }
                });
            })
	 ...
	 _.DrawHelperWidgetBTN12 = (function() {
        ...
        function _(drawHelper, options) {
            ...
            var toolbar12 = document.createElement('DIV');
            toolbar12.className = "toolbar12";
            options.container.appendChild(toolbar12);

            function addIcon(id, url, title, callback) {
			var thakoimitholes=document.getElementById("toolbar12");
				//alert("de paizei na koimitho: "+thakoimitholes.name);
				thakoimitholes.onclick = callback;
                //var div = document.createElement('submit');
                //--div.className = 'flamebutton';
	...
	//DrawHelperWidgetBTNreqcorine
	_.DrawHelperWidgetBTNreqcorineVIS = (function() {
        ...
        function _(drawHelper, options) {
            ...
            var toolbar11requestcorineVIS = document.createElement('DIV');
            toolbar11requestcorineVIS.className = "toolbar11requestcorineVIS";
            options.container.appendChild(toolbar11requestcorineVIS);

            function addIcon(id, url, title, callback) {
			var thakoimitholes=document.getElementById("toolbar11requestcorineVIS");
				//alert("de paizei na koimitho: "+thakoimitholes.name);
				thakoimitholes.onclick = callback;
                //var div = document.createElement('submit');
                //--div.className = 'flamebutton';
            ...
            var div = document.createElement('DIV');
            div.className = 'divider';
            toolbar11requestcorineVIS.appendChild(div);
            enhanceWithListeners(this);
	...
	_.DrawHelperWidgetBTNreqcorine = (function() {
        ...
        function _(drawHelper, options) {
            ...
            var toolbar11requestcorine = document.createElement('DIV');
            toolbar11requestcorine.className = "toolbar11requestcorine";
            options.container.appendChild(toolbar11requestcorine);

            function addIcon(id, url, title, callback) {
			var thakoimitholes=document.getElementById("toolbar11requestcorine");
				//alert("de paizei na koimitho: "+thakoimitholes.name);
				thakoimitholes.onclick = callback;
                //var div = document.createElement('submit');
                //--div.className = 'flamebutton';
            ...
            var div = document.createElement('DIV');
            div.className = 'divider';
            toolbar11requestcorine.appendChild(div);
            enhanceWithListeners(this);
    ...
    _.DrawHelperWidget = (function() {
        ...
        function _(drawHelper, options) {
            ...
            addIcon('clear', options.clearIcon, 'Remove all primitives', function() {
                //alert('BEFORE scene is '+scene.primitives.length);
				//scene.primitives.removeAll();
				//DrawHelper.ExtentPrimitive.prototype.destroy();//DOULEVEI
				...
				/*for (var i = 0; i < length; ++i) {
					...
					if(i != 0){
						...
						var p = primitives1.get(i);
						//scene.primitives.remove(extentPrimitive);	
						//scene.primitives.get(i).show=false;
					
						scene.primitives.remove(p);
					}
					console.log(i+": AFTER lenght is "+primitives1.length);
				}*/
    ...
    _.prototype.addToolbar = function(container, options) {
        ...
        return new _.DrawHelperWidget(this, options);
		    ...
		    _.prototype.addBTN = function(container, options) {
        options = copyOptions(options, {container: container});
        return new _.DrawHelperWidgetBTN(this, options);
    }
			_.prototype.addBTNperimeter = function(container, options) {
        options = copyOptions(options, {container: container});
        return new _.DrawHelperWidgetBTNperimeter(this, options);
    }
			    _.prototype.addBTN12 = function(container, options) {
        options = copyOptions(options, {container: container});
        return new _.DrawHelperWidgetBTN12(this, options);
    }
		    _.prototype.addBTNreqcorine = function(container, options) {
        options = copyOptions(options, {container: container});
        return new _.DrawHelperWidgetBTNreqcorine(this, options);
    }
			_.prototype.addBTNreqcorineVIS = function(container, options) {
        options = copyOptions(options, {container: container});
        return new _.DrawHelperWidgetBTNreqcorineVIS(this, options);
    }
    ...
    function createTooltip(frameDiv) {
	    ...
	    function createTooltipLav(frameDiv) {

        var tooltip = function(frameDiv) {

            var div = document.createElement('DIV');
            div.className = "loggingpuretest";
var les=document.getElementById("New Forest1");
les.appendChild(div);
           // var arrow = document.createElement('DIV');
           // arrow.className = "twipsy-arrow";
           // div.appendChild(arrow);
    ...
    function getDisplayLatLngString(cartographic, precision) {
		//var lonDegrees = Cesium.Math.toDegrees(cartographic.longitude);
		//var latDegrees = Cesium.Math.toDegrees(cartographic.latitude);
		//alert("ragizei i karida mou "+lonDegrees+": "+latDegrees);
